...
def make_records():
    ...
    if Product.select().where(Product.product_name == "Trouser"):
        ...
        tags=[],
        catalog_id=1)
    trouser = Product.get(Product.product_name == "Trouser")
    add_tag(trouser, "Domestic")
    add_tag(trouser, "Clothes")
    if Product.select().where(Product.product_name == "Cluedo"):
        ...
        tags=[],
        catalog_id=1)
    cluedo = Product.get(Product.product_name == "Cluedo")
    add_tag(cluedo, "Domestic")
    add_tag(cluedo, "Games")
    if Product.select().where(Product.product_name == "Cluedo"):
        None
    else: Product.create(
        product_name="Cluedo",
        description="Cluedo the Dutch edition",
